...
def inference(x, n_in, n_hiddens, n_out):
    ...
    def batch_normalization(shape, x):
        eps = 1e-8  # イプシロン。0 div にならないようにしている。
        beta = tf.Variable(tf.zeros(shape))  # gamma, beta がパラメータ
        gamma = tf.Variable(tf.ones(shape))  # gamma, beta がパラメータ
        mean, var = tf.nn.moments(x, [0])  # 平均、分散 (moments 便利！)
